...
class Cate extends Component {
    ...
    render() {
       let arr=this.props.cateTrees.length>0?this.props.cateTrees[this.state.tag].children:[];
        return (
            <div className='Cate'>
                <div className="top">
                    ...
                    <ul className="left">
                        {
                            this.props.cateTrees.length > 0 ? (
                                this.props.cateTrees.map((item, index) => {
                                    return (
                                        <li onClick={this.changeTag.bind(this, index)} className={this.state.tag === index ? 'select' : ''} key={item.id}>
